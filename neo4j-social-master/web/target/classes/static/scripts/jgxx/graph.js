...
function showGraph(queryName){
	...
	$.get('../jgxx/jgxxControllerGraph/'+queryName,{ts:new Date().getTime()}, function(graph) {
	      ...
	      .style("fill",function(node){
		    ...
		    /*   if(link!=undefined){
			      if(link.source.title==node.title && node.title==chaxuntext){
			          ...
			          if(node.label=="officer"){
			          	 color="#2172B7";
			          }else if(node.label=="jg"){
			        	  if(node.jglscj=='03')
			        	  {color="#48F1EB";}
			        	  if(node.jglscj=='04')
			        	  {color="#48F1EB";}
	       ...
	       .style('stroke',function(node){ //
	        ...
	        .on("click",function(node){
	         ...
	         .on("dblclick",function(d,i){
		        ...
		        else if(d.label=="jg"){//机构
		        	if(d.jglscj!="05"){//机构隶属层级
		        	$.get('../jgxx/jgxxGraph/'+d.jgbm+"/"+d.index+"/"+nodes.length,{ts:new Date().getTime()}, function(showGraph){
		        		 if(showGraph){
				        	 if(showGraph.nodes){
		        	 ...
		        	 $.get('../officer/officerGraph/'+d.jgbm+"/"+d.index+"/"+nodes.length,{ts:new Date().getTime()}, function(showGraph){
				         if(showGraph){
				        	 if(showGraph.nodes){
				        		 for(var index in showGraph.nodes){
								     nodes.push(showGraph.nodes[index]);
